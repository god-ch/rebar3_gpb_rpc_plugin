-module(rebar3_gpb_grpc_compiler).
...
compile(AppInfo, State) ->
    ...
    {ok, GpbOpts0} = dict:find(gpb_grpc_opts, Opts),
    ...
    TargetErlDir = filename:join([AppOutDir, proplists:get_value(o_erl, GpbOpts0, ?DEFAULT_OUT_ERL_DIR)]),
    TargetHrlDir = filename:join([AppOutDir, proplists:get_value(o_hrl, GpbOpts0, ?DEFAULT_OUT_HRL_DIR)]),
    ...
    rebar_api:debug("reading proto files from ~p, generating \".erl\" to ~p " "and \".hrl\" to ~p", [SourceDirs, TargetErlDir, TargetHrlDir]),
    ...
    Protos =
        case proplists:get_value(f, GpbOpts0) of
            undefined ->
                FoundProtos;
            WantedProtos ->
    ...
    GpbOpts = [{rename, {msg_name, snake_case}}, {rename, {msg_fqname, base_name}}] ++ remove_plugin_opts(
    ...
    Modules = compile(Protos, TargetErlDir, GpbOpts, Protos, []),
    ok = gen_service_behaviours(Modules, TargetErlDir, GpbOpts, State),
    ok.
...
clean(AppInfo, State) ->
    ...
    AppOutDir = rebar_app_info:out_dir(AppInfo),
    Opts = rebar_app_info:opts(AppInfo),
    {ok, GpbOpts} = dict:find(gpb_grpc_opts, Opts),
    ModuleNamePrefix = proplists:get_value(module_name_prefix, GpbOpts, ?DEFAULT_MODULE_PREFIX),
    ModuleNameSuffix = proplists:get_value(module_name_suffix, GpbOpts, ?DEFAULT_MODULE_SUFFIX),
    TargetErlDir = filename:join([AppOutDir, proplists:get_value(o_erl, GpbOpts, ?DEFAULT_OUT_ERL_DIR)]),
    TargetHrlDir = filename:join([AppOutDir, proplists:get_value(o_hrl, GpbOpts, ?DEFAULT_OUT_HRL_DIR)]),
    ProtoFiles = find_proto_files(AppDir, DepsDir, GpbOpts),
    rebar_api:debug("found proto files: ~p", [ProtoFiles]),
    GeneratedRootFiles = [ModuleNamePrefix ++ filename:rootname(filename:basename(ProtoFile)) ++ ModuleNameSuffix || ProtoFile <- ProtoFiles],
    GeneratedErlFiles = [filename:join([TargetErlDir, F ++ ".erl"]) || F <- GeneratedRootFiles],
    GeneratedHrlFiles = [filename:join([TargetHrlDir, F ++ ".hrl"]) || F <- GeneratedRootFiles],
    GeneratedServiceErlFiles = find_gen_service_behaviours(TargetErlDir),
    rebar_api:debug("deleting [~p, ~p]", [GeneratedErlFiles, GeneratedHrlFiles]),
    rebar_file_utils:delete_each(GeneratedErlFiles ++ GeneratedHrlFiles ++ GeneratedServiceErlFiles).
...
discover(AppDir, SourceDir, Opts) ->
    ...
    rebar_utils:find_files(filename:join([AppDir, SourceDir]), SourceExtRe, Recursive).

compile([], _TargetErlDir, _GpbOpts, _Protos, Modules) -> Modules;
compile([Proto | Rest], TargetErlDir, GpbOpts, Protos, Modules) ->
    ModuleNamePrefix = proplists:get_value(module_name_prefix, GpbOpts, ?DEFAULT_MODULE_PREFIX),
    ModuleNameSuffix = proplists:get_value(module_name_suffix, GpbOpts, ?DEFAULT_MODULE_SUFFIX),
    Target = target_file(Proto, ModuleNamePrefix, ModuleNameSuffix, TargetErlDir),
%%    io:format("==========change time Target:~p~n", [Target]),
%%    case filelib:last_modified(Target) < filelib:last_modified(Proto) of
%%        true ->
    {ok, Module} = compile(Proto, Target, GpbOpts),
    ...
    rebar_api:debug("protos that include ~p: ~p", [Proto, Deps0]),
    %% now touch the targets of each of the deps so they get remade again
    change_file_time(Deps0, ModuleNamePrefix, ModuleNameSuffix, TargetErlDir),

    compile(Rest, TargetErlDir, GpbOpts, Protos, [Module | Modules]).
%%        false ->
%%            compile(Rest, TargetErlDir, GpbOpts, Protos, Modules)
%%    end.

change_file_time(Deps, ModuleNamePrefix, ModuleNameSuffix, TargetErlDir) ->
    lists:foreach(
        fun(Dep) ->
            DepTarget = target_file(Dep, ModuleNamePrefix, ModuleNameSuffix, TargetErlDir),
            io:format("==========change time DepTarget:~p~n", [DepTarget]),
            ...
            Seconds = calendar:datetime_to_gregorian_seconds(filelib:last_modified(Dep)) - 60,
            _ = file:change_time(DepTarget, calendar:gregorian_seconds_to_datetime(Seconds)),
...
filter_included_proto(Source, Dep, GpbOpts) ->
    {ok, Defs, _Warnings} = gpb_compile:file(filename:basename(Source), GpbOpts ++ [to_proto_defs, return]),
    Imports =
        lists:map(
            fun(Import0) ->
            ...
            lists:usort(gpb_parse:fetch_imports(Defs))
        ),
...
compile_file(Source, GpbOpts) ->
    case gpb_compile:file(filename:basename(Source), [binary, return] ++ GpbOpts) of
        {ok, Module, Compiled} ->
            {module, _} = code:load_binary(Module, Source, Compiled),
            {ok, Module};
        {ok, Module, Compiled, _Warnings} ->
            {module, _} = code:load_binary(Module, Source, Compiled),
            {ok, Module};
        {error, Reason} ->
            ReasonStr = gpb_compile:format_error(Reason),
...
ensure_dir(OutDir) ->
    ...
    case filelib:ensure_dir(filename:join(OutDir, "dummy.beam")) of
        ok -> ok;
        {error, eexist} ->
            rebar_utils:abort("unable to ensure dir ~p, is it maybe a broken symlink?", [OutDir]);
...
-spec remove_plugin_opts(proplists:proplist(), [ipath | recursive]) -> proplists:proplist().
...
find_proto_files(AppDir, DepsDir, GpbOpts) ->
    lists:foldl(
        fun
            ({deps, SourceDir}, Acc) ->
                Acc ++ rebar_utils:find_files(filename:join(DepsDir, SourceDir), ".*\.proto\$");
            (SourceDir, Acc) ->
                Acc ++ rebar_utils:find_files(filename:join(AppDir, SourceDir), ".*\.proto\$")
        end,
        [],
...
gen_service_behaviours([], _TargetErlDir, _GpbOpts, _State) -> ok;
gen_service_behaviours([GPBModule | T], TargetErlDir, GpbOpts, State) ->
    gen_service_behaviour(GPBModule, TargetErlDir, GpbOpts, State),
    gen_service_behaviours(T, TargetErlDir, GpbOpts, State).

gen_service_behaviour(GPBModule, TargetErlDir, _GpbOpts, State) ->
    case erlang:function_exported(GPBModule, get_service_names, 0) of
        true ->
            ServiceNames = GPBModule:get_service_names(),
            lists:foreach(
                fun(S) ->
                    {{_, Name}, Methods} = GPBModule:get_service_def(S),
                    ModuleName = list_snake_case(atom_to_list(S)),
                    MethodList = lists:reverse(lists:foldl(
                        fun(M, Acc) ->
                            MA = lists:flatten([
                                [
                                    [
                                    ...
                                    ] || {X, Y} <- maps:to_list(M), X =/= opts],
                                {message_type, GPBModule:msg_name_to_fqbin(maps:get(input, M))}
                            ]),
                            [MA | Acc]
                        end,
                        [],
                    ...
                    Service = [
                        {out_dir, TargetErlDir},
                        {pb_module, atom_to_list(GPBModule)},
                        {unmodified_service_name, atom_to_list(Name)},
                        {module_name, ModuleName},
                        {methods, MethodList}
                    ],
                    TemplateName = to_template_name(all),
                    rebar_templater:new(TemplateName, Service, true, State)
                end,
                ServiceNames
            ),
            ok;
...
find_gen_service_behaviours(TargetErlDir) ->
    rebar_utils:find_files(TargetErlDir, "_bhvr.erl\$") ++ rebar_utils:find_files(TargetErlDir, "_client.erl\$").
